...
# docker buildx build --tag cornwall --file .pi/Dockerfile --platform linux/arm64 --cache-from cornwall:cache --cache-to cornwall:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar cornwall
...
sudo bash -c "echo cornwall > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/cornwall.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/cornwall.img
